...
def generateFitRegDataFig(paraName, dataDir, figDir,modelV=None):
    ...
    if os.path.isfile(irrDatDir + 'irregularSpikePairs_vs_deltaT_differentFreqs_%s.npy' % paraName):
        irrData = True
        modelNew = np.load(irrDatDir + 'irregularSpikePairs_vs_deltaT_differentFreqs_Apr26_%s.npy' % paraName)
        modelOld = np.load(irrDatDir + 'irregularSpikePairs_vs_deltaT_differentFreqs_%s.npy' % paraName)
    ...
    params = {'axes.labelsize': 11, 'axes.titlesize': 10, 'font.size': 12, 'xtick.labelsize': 11, 'ytick.labelsize': 11, 'figure.figsize': fig_size,  # 'savefig.dpi': 600,
    ...
    lineStyles = ['-',':','-.','--']
    xlimits = [[-260,260],[-260,260],[-260,260],[-100,100],[-100,100],[-100,100]]
    # third sub-plot #######################################################
    for i in range(len(sChange.stimFrequencies)):
        ax4 = plt.subplot(gs[i])

        # title
        ax4.set_title('%s spikes/second' % sChange.stimFrequencies[i])
        ...
        #for i in range(len(sChange.stimFrequencies)):
        ax4.plot(synChange[:, 0] * 1000., synChange[:, i + 1] * 100.,c = 'C2', lw=2, label='model fit to \nregular data')
        ax4.plot(modelNew[:, 0] * 1000., modelNew[:, (14 + i)] * 100.,  c='C5', lw=2, label='model prediction \nfor irregular stim.')
        ...
        ax4.set_xlim(xlimits[i][0],xlimits[i][1])
        ax4.set_ylim(ylowerLimit,yupperLimit)
        # legends and labels
        if i==0:
            plt.legend(loc=(0.52,0.73), frameon=False)
        ...
        if i>=3:
            ax4.set_xlabel(r'time lag $\Delta t_{\rm STDP}$ (ms)')
        if not (i%3):
            ax4.set_ylabel('change in synaptic strength (%)')

    # # third sub-plot #######################################################
    # ax4 = plt.subplot(gs[1])
    #
    # # title
    # ax4.set_title('model prediction for irregular data')
    #
    # # diplay of data
    # ax4.axhline(y=100, ls='--', color='0.7', lw=2)
    # ax4.axvline(x=0, ls='--', color='0.7', lw=2)
    ...
    #     ax4.plot(modelNew[:, 0] * 1000., modelNew[:, (12 + i)] * 100., lw=2, label=str(sChange.stimFrequencies[i])+ ' spk/s')
    #     #if i<4:
    #     #    ax4.plot(modelOld[:, 0] * 1000., modelOld[:, (10 + i)] * 100., lw=2, ls='--', label='old'+str(sChange.stimFrequencies[i]))
    # # ax4.plot(synChange[:, 0] * 1000., synChange[:, 2] * 100.)
    # # ax4.plot(synChange[:, 0] * 1000., synChange[:, 3] * 100.)
    # print(modelNew[:, 0] * 1000.)
    # #pdb.set_trace()
    # # if 'old'==oldNew:
    # # ax2.plot(-model5Hz[:,3]+2.*synapticChange.D*1000.,model5Hz[:,18]*100.)
    # # else:
    ...
    # ax4.set_ylim(ylowerLimit,yupperLimit)
    # # legends and labels
    # plt.legend(loc=(0.74, 0.4), frameon=False)
    # leg = plt.gca().get_legend()
    # ltext = leg.get_texts()
    # plt.setp(ltext, fontsize=9)
    # ax4.yaxis.set_major_locator(majorLocator_x)
    # ax4.set_xlabel(r'time lag $\Delta t$ (ms)')
    # #ax4.set_ylabel('change in synaptic strength')
    ...
    ##########################
    fname = 'fig_regularIrregularFrequencies_moreFrequs_%s_v0' % paraName
