...
class synUtils(): # synUtils(thetaD,thetaP,nonlinear,Npresentations,w0)
    ...
    def generateVenFig(self, paraName, figName=None, modelV = None):
        ...
        allData = pickle.load( open( self.dataDir + "allIrregularData.p", "rb" ) )
        ...
        if os.path.isfile(self.figDir+'irregularSpikePairs_vs_deltaT_differentFreqs_%s.npy' % paraName):
            ...
            modelBurstsNew = np.load(self.figDir+'irregularBurstSpikePairs_vs_deltaT_differentFreqs_%s.npy' % paraName)
            modelIndNew = np.load(self.figDir+'irregularIndividualSpikePairs_vs_deltaT_differentFreqs_%s.npy' % paraName)
        ...
        for n in range(len(sChange.stimFrequencies)):
            ...
            for i in range(len(deltaT)):
                ...
                elif self.modelVersion == 'binary':
                    synChange[i, n+1] = sChange.synChange
        ...
        ax0.errorbar(stdp1binned[:, 0], stdp1binned[:, 1]*100., yerr=stdp1binned[:, 2], fmt='o-', markeredgecolor='C0')
        ...
        ax_inset.errorbar(stdp1binned[:, 0], stdp1binned[:, 1]*100., yerr=stdp1binned[:, 2], fmt='o-', markeredgecolor='C0')
        ...
        if irrData:
            ...
            ax_inset.plot(modelIndNew[:,0]*1000.,modelIndNew[:,10]*100.)
            # if oldNew == 'old':
        ...
        ax1.errorbar(stdp3binned[:, 0], stdp3binned[:, 1]*100., yerr=stdp3binned[:, 2], fmt='o-', markeredgecolor='C0', label=r'exp. data: reg. pairs')
        ax1.plot(synChange[:, 0] * 1000., synChange[:, 2] * 100., label=r'model fit: reg. pairs')
        ax1.errorbar(stdpIrr3HzBinned[:, 0] * 1000., stdpIrr3HzBinned[:, 2] * 100., xerr=stdpIrr3HzBinned[:, 1] * 1000., yerr=stdpIrr3HzBinned[:, 3] * 100., fmt='o-', label=r'exp. data: irr. pairs')
        if irrData:
        ...
        if irrData:
            ax2.plot(modelNew[:,0]*1000.,modelNew[:,12]*100.,c='C3')
        ...
        if irrData:
            ax3.plot(modelNew[:,0]*1000.,modelNew[:,13]*100.,c='C3')
